...
Copyright (C) 2004-2007, 2009 Paul Puzyrev, Sergei Larionov. www.minibb.net
...
if ($user_id == 0 or $user_id == 1) {
    ...
    if (isset($_POST['user_viewemail']))
        $user_viewemail = $_POST['user_viewemail'] + 0;
    $showemailDown = makeValuedDropDown(array(
        0 => $l_no,
        1 => $l_yes
    ), 'user_viewemail');
    if (isset($_POST['user_sorttopics']))
        $user_sorttopics = $_POST['user_sorttopics'] + 0;
    $sorttopicsDown = makeValuedDropDown(array(
        0 => $l_newAnswers,
        1 => $l_newTopics
    ), 'user_sorttopics');
    if (!isset($_POST['language']))
        ...
        $language = operate_string($_POST['language']);
    $languageDown = makeValuedDropDown($glang, 'language');
    
    if (isset($user_usr) and $step == 0)
        $login = $user_usr;
    $userTitle = $l_newUserRegister;
    
    if ($step == 1) {
        require_once("metacaptcha/metacaptcha_lib.php");
	    $verify = metacaptcha_verify($_POST['metacaptchaField'],"Hello");
	    //the CAPTCHA is correct
	    if(!$verify){

	      echo "doesn't pass metacaptcha verify, please contant admin";
	    }
        ...
        if (db_simpleSelect(0, $Tu, $dbUserId, $dbUserId, '=', 1) and !db_simpleSelect(0, $Tu, $dbUserId, $caseComp . "({$dbUserSheme['username'][1]})", '=', strtolower(${$dbUserSheme['username'][2]})) and !db_simpleSelect(0, $Tu, $dbUserId, $caseComp . "({$dbUserSheme['user_email'][1]})", '=', strtolower(${$dbUserSheme['user_email'][2]})) and ${$dbUserSheme['username'][2]} != $admin_usr and strtolower(${$dbUserSheme['user_email'][2]}) != strtolower($admin_email)) {
            ...
            if ($correct == 0) {
                ...
                $insa = array(
                    ...
                    $dbUserDate,
                    $dbUserSheme['user_password'][1],
                    $dbUserSheme['user_email'][1],
                    $dbUserSheme['user_viewemail'][1],
                    $dbUserSheme['user_sorttopics'][1],
                    $dbUserSheme['language'][1],
                    $dbUserAct
                );
                ...
                $warning = "<span class=\"warning\">{$l_userErrors[$correct]}</span>";
                $title   = $l_errorUserData;
                $tpl     = makeUp('user_dataform');
                if (isset($closeRegister) and $closeRegister == 1)
                    $tpl = preg_replace("#<!--PASSWORD-->(.*)<!--/PASSWORD-->#is", '', $tpl);
            }
        } else {
            $action = 'register';
            if (file_exists($pluginsFile))
                include($pluginsFile);
            $title   = $l_errorUserData;
            $warning = '<span class=warning>' . $l_errorUserExists . '</span>';
            $tpl     = makeUp('user_dataform');
            if (isset($closeRegister) and $closeRegister == 1)
                $tpl = preg_replace("#<!--PASSWORD-->(.*)<!--/PASSWORD-->#is", '', $tpl);
        }
        ...
        echo load_header();
        echo ParseTpl($tpl);
        require "metacaptcha/metacaptcha_lib.php"; //Link to metacaptcha_lib.php
